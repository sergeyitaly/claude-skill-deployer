...
| Versions | Theme |
|----------|--------|
| **1.0.153** | A value-audit's hook-latency numbers (up to 29s on SessionStart, 200-450ms on every prompt) checked out as real and got fixed at the root — but chasing the fix for one of them (mcp-force/mcp-gate never re-syncing) turned up a much more serious, currently-live bug: an empty legacy-filename argument made a hook-migration filter match and silently delete *every* unrelated hook in the same category, on every extension activation |
...
**Summary:** A value-audit report's hook-latency numbers — up to 29s on a SessionStart hook, 200-450ms added to every prompt via two other hooks, and mcp-force/mcp-gate never appearing in `hook-health.jsonl` — all checked out against this project's own telemetry and got fixed at the root. Writing a regression test for the mcp-force/mcp-gate fix then surfaced a far more serious, currently-live bug in the same shared hook-registration code: an empty legacy-filename argument silently deleted every unrelated hook in the same category on every single extension activation.

**Theme:** Hook-pipeline reliability — three real, measured latency/visibility bugs fixed, plus one severe silent-data-loss bug found while regression-testing the third fix, not left unverified.

### Fixed

- **`official-skills` SessionStart hook could take up to 29 seconds.** `fetchRemoteHeadSha()` (`officialSkillsSync.ts`) already had a 15s timeout on its `git ls-remote` call from a prior fix, but `fetchUpstreamSkillNames()` — the second network call in the same flow, a GitHub API `fetch()` only reached once the remote SHA has actually changed — had no timeout at all. Confirmed live: a recorded call took 28,879ms, longer than the git call's own cap, meaning the untimed `fetch()` was the actual slow leg. Added a matching `AbortController`-based 15s timeout with a clear "timed out after 15s" error.
- **`budget` hook added 200-455ms to every prompt.** `handleBudget()` (`hookHandlers.ts`) called `usageCost.ts`'s `computeTodayCreditUsage()`, which does a full disk walk and parse of every transcript file under `~/.claude/projects` on every call, uncached. New `computeTodayCreditUsageCached()` (`transcriptUsageIndex.ts`) reuses the same fingerprint-invalidated cache `agentOps.ts` already relies on for the identical underlying data — a repeated call in the same session now costs a cheap mtime check instead of a full re-parse.
- **`practical-focus` hook added 124-164ms to every prompt.** `detectInfraSignals()` → `walkForPattern()` (`hookHandlers.ts`) recursively scanned the whole workspace tree with **no `node_modules`/`.git` exclusion at all** — unlike `collectRelativePaths()` (`skillOps.ts`) right in the same codebase, which already has an `EXCLUDE_DIRS` list for exactly this. Confirmed live: this repo's own `extension/node_modules` alone has 14,421 files. Exported `EXCLUDE_DIRS` from `skillOps.ts` and reused it in `walkForPattern()`, plus added a 60s TTL cache (matching `detectRelevantSkills()`'s existing pattern) since this runs on every prompt. Combined with the budget fix above, `handlePromptContext()`'s per-prompt cost measured in this repo dropped from ~150-190ms to ~6-56ms end to end.
- **`mcp-force`/`mcp-gate` hooks never re-synced after the hook server's port changed, unlike every other hook category.** `installMcpForceHook()`/`installMcpGateHook()` were only ever called once, from the manual "Enable MCP-Force Mode" command — never again on later activations, unlike `installDirCacheGuardHook()` and friends, which already call through unconditionally on every activation specifically so a stale port gets rewritten. `extension.ts` now does the same for mcp-force/mcp-gate, gated on the workspace already having MCP-Force permissions active (never auto-enables the feature).
- **A hook-migration filter could delete every unrelated hook in the same category, silently, on every activation.** `ensureSessionStartHookRegistered()`/`ensurePostToolHookRegistered()`/`ensurePreToolHookRegistered()` (`hookOps.ts`) each filter out "legacy" hook entries via `command.includes(legacyFilename)` — but `installMcpGateHook()`, `installDirCacheGuardHook()`, `installCliLoopGuardHook()`, and `installMcpErrorGuardHook()` all pass `legacyFilename: ""` (they're brand-new hooks with no legacy name to migrate from), and `"anything".includes("")` is always `true` in JavaScript. The filter treated *every* existing matcher in that hook category as legacy and deleted it, keeping only the one hook being installed. Confirmed live and reproduced directly: `installMcpGateHook()` deleted a pre-existing `official-skills` SessionStart hook outright (this project's own `settings.json` only survived because `official-skills` happened to be reinstalled *after* `mcp-gate` in its real history); `installDirCacheGuardHook()` — already called unconditionally on every extension activation — deletes the unrelated `skill-invoke` `PreToolUse` matcher every single time, with nothing to restore it since attribution hooks are only reinstalled when a presence check reports them *missing*. All three functions now skip the legacy-removal filter entirely when `legacyFilename` is empty.
- 12 new regression tests across `officialSkillsSync.test.ts` (timeout, using `vi.useFakeTimers()` so the test doesn't wait 15 real seconds), `transcriptUsageIndex.test.ts` (budget caching), `detectInfraSignals.test.ts` — new file (exclusion + caching), and `hookOps.test.ts` (stale-port re-sync plus two dedicated tests proving the empty-legacy-filename fix). Every fix was verified by reverting it and confirming its test fails with the exact reported symptom before restoring it. Full extension suite: 117 files, 1010 tests, all passing; `tsc --noEmit` clean.
...
- **`usageStats.compute.test.ts`'s `resetMisattributedData` test hardcoded `ts: "2026-06-12T..."` for its rows.** `resetMisattributedData()` runs `pruneRunsJsonl()` (90-day retention) after filtering collector-transcript rows; once real time passed the 90-day mark from that hardcoded date, the prune step deleted both the transcript row *and* the hook row the test expects to survive, failing an assertion unrelated to what the test actually verifies. CI caught this the same day it aged out. Rows now use a timestamp relative to `Date.now()`.

---

## [1.0.150] - 2026-09-11

**Summary:** A pasted extension-value-audit report claimed a dead SessionStart hook (`skill-gap-detector.js` pointing at an old version), a phantom, never-installed skill polluting adoption/cost data, and specific telemetry-file sizes. None of those exact facts reproduced against this repo's actual `.claude/settings.json` or `.claude/learning/*` — the file paths, line counts, and disabled-skill counts the report cited don't match what's on disk here. Rather than "fixing" data that doesn't exist, the underlying code was audited for the *class* of bug each finding implied, and two real, general ones were found and fixed.

**Theme:** Verify-before-fix — a report's specific numbers can go stale (or come from a different workspace) even when the mechanism it's pointing at is real; this round fixed the mechanisms, not the numbers.

### Fixed

- **A hook script deleted outright (not renamed) is never cleaned out of an existing workspace's `settings.json`.** Every `ensureXHookRegistered()` helper in `hookOps.ts` migrates a *renamed* legacy hook (e.g. `session-size-watch.js` → `prompt-context`) via a `legacyFilenames` parameter, but `skill-gap-detector.js`, `session-apply.js`, `task-skill-focus.js`, `file-split-advisor.js`, and `commit-cost-record.js` were removed with no replacement in the earlier "Dead hook removal" cleanup — none of them appear in any `legacyFilenames` list. A workspace that had one of these configured before that cleanup keeps calling a file the installed extension no longer ships, failing silently (`MODULE_NOT_FOUND`) on every trigger forever. New `removeDeadHookScriptReferences()` scans every hook category in `settings.json` for these five filenames and strips them; wired into the same unconditional per-activation hook-migration pass as `installTerminalWatchHook()`.
- **Reading any file under `skills_library/<name>/` was counted as that skill being invoked, even when it was never installed.** `skillFromPath()` (`hookHandlers.ts`) matched `skills_library` with the same pattern used for the actually-active skill directories (`.claude/skills/`, `.cursor/skills-cursor/`, `.agents/skills/`) — but `skills_library/` is this extension's own deployment source catalog, not a location any agent is ever instructed to load a skill from. A plain Read of a reference file, script, or manifest entry while browsing/maintaining the catalog (e.g. via `skill-creator` or `skill-official-updater`) got written to `runs.jsonl` and the adoption funnel as a genuine skill invocation, inflating both with skills that were never actually installed or used. The `skills_library` match is now checked separately and only counted when the same skill also exists under `.claude/skills/<name>/` in the workspace; otherwise it's logged as an ordinary tool read like any other Read call.
- 4 new regression tests (`hookOps.test.ts` ×2, `hookHandlers.test.ts` ×2) covering the dead-hook cleanup and the installed-vs-catalog skill distinction.
