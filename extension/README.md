...
6. Click **"Show Skill Usage Report"** (graph icon, also in the status bar)
   ...
   based on `.claude/learning/runs.jsonl`.
...
| `Claude Skills: Show Skill Usage Report` | Opens a per-skill usage/KPI report (runs, success rate, last used, rating, suggested skills, misused/removal candidates) based on `.claude/learning/runs.jsonl`. |
