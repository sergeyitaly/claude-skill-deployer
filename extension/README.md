...
- **Profile init (role + branch)** — on a new git branch with no saved profile,
  the extension can prompt to initialize skills for your **position** (DevOps, QA,
  Frontend, etc.). It writes `.claude/learning/skills-catalog.json` (live skill
  inventory) and `.claude/learning/profile-init-request.json`; your AI agent runs
  the **`profile-init` skill**, picks skills from the catalog (no fixed role map),
  and writes `.claude/profile.local.json` (gitignored). The extension auto-installs
  the chosen skills and saves the branch profile locally. See [Profile init](#profile-init-role--branch-agent-driven) below.
...
10. **New branch?** When prompted (or via **Init Profile for Current Branch**), set your position, then ask any synced AI agent to run **`profile-init`** — it writes `.claude/profile.local.json` and the extension installs your skill set.

## Profile init (role + branch, agent-driven)

Personal skill setup when you start work on a **new git branch**. Your **position** is stable; the **skill list** is chosen by the AI agent from whatever the extension exposes in the catalog today (no hardcoded role map).

### Steps

1. **Set position** (once per workspace) — **Set Your Position** → saved to `.claude/position.local.json`.
2. **Start init** — automatic prompt on branch switch, or **Init Profile for Current Branch**.
3. **Agent interview** — run **`profile-init`** in Claude, Cursor, Kiro, or Copilot (after multi-agent sync). The agent reads:
...
4. **Agent output** — `.claude/profile.local.json` with `skills[]` and optional `rationale`.
5. **Extension apply** — file watcher (or **Apply Local Profile**) installs skills, syncs agents, updates `~/.claude/learning/branch-profiles.json`.

### Multi-agent (Cursor, Kiro, Copilot)

| Layer | Agent-specific? |
|---|---|
| VS Code prompts, catalog, auto-apply | No |
| Writing `profile.local.json` | No — any agent with workspace write access |
| Installing chosen skills | No — extension only |

With `claudeSkills.agents.syncWorkspaceToAll` (default), `profile-init` is copied to:

- `.cursor/skills/profile-init/SKILL.md`
- `.kiro/skills/profile-init/SKILL.md`
- `.github/instructions/profile-init.instructions.md`

Paths in the skill always reference **`.claude/learning/`** and **`.claude/profile.local.json`** — the extension’s contract, not Claude-only. **Init Profile for Current Branch** syncs the skill to all enabled agents; after the automatic new-branch prompt alone, run **Sync Workspace Skills to All Agents** if your primary agent is not Claude.

Example prompt: *“Initialize my skill profile for this branch using profile-init.”*

### Local files (gitignored via `.git/info/exclude`)
...
| `Claude Skills: Init Profile for Current Branch` | Starts agent-guided profile init: refreshes skill catalog, installs `profile-init` skill, writes request file for the AI agent. |
