...
- **Team branch profiles in git** — commit a shared layout per branch in
  ...
  Teammates get the same effective skill set on branch switch (personal
  `skillOverrides` still apply on top).
- **Attribution v2 hooks** — **Enable Attribution Hooks (v2)** logs each
  Skill/Read tool invoke to `runs.jsonl` for per-skill cost attribution
  without equal-split transcript guessing.
