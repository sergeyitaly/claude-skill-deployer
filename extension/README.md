...
7. Optionally run **"Enable Session Size Notifications"** (bell icon) to get
   a one-time nudge to run `/compact` or `/clear` when a Claude Code
   session's transcript grows large.
...
| `Claude Skills: Enable Session Size Notifications` | Installs a `UserPromptSubmit` hook that nudges you to run `/compact` or `/clear` once a session's transcript gets large. |
