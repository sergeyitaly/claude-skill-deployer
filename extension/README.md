...
- **Multi-agent clones (auto-sync by default)** — one `skills_library/` fans
  ...
  `claudeSkills.agents.syncWorkspaceToAll` is `true` (default), every
  workspace install — generate, checkbox, single-skill install, branch profile
  apply — copies to **all enabled agent paths**. Changes under
  `.claude/skills/` also sync to other agents via a file watcher.
  `.claude/skills/` stays the git-tracked source of truth.
