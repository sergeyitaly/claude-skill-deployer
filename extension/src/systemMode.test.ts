...
const readyFreshCycle: PipelineCycleTimestamps = {
...
describe("systemMode", () => {
  it("enters safe mode when pipeline is stale or confidence is low", () => {
    expect(resolveSystemMode(health({ confidenceScore: 0.4 }), target, readyFreshCycle)).toBe("safe");
    expect(
      resolveSystemMode(health({ confidenceScore: 0.8, reliable: true }), target, {
  ...
  it("allows auto-apply only in normal mode with fresh pipeline", () => {
    const ctx = buildSystemModeContext(
    ...
    expect(ctx.pipelineFresh).toBe(true);
