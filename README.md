...
- **Auto-started on activation** â€” deployed and registered for Claude/Cursor/Kiro automatically. The server binary is auto-synced on every activation so extension updates propagate without a manual "Enable" step.
- **`edit_file` tool (v1.2)** â€” targeted string-replace edits. `old_string` must match exactly once; errors on ambiguous or missing matches. Prefer over `write_file` for surgical changes.
- **Session read/dir cache** â€” `read_file` skips re-reads of unchanged files (mtime guard); `list_directory` returns cached listings within the same session. Both caches invalidate on writes, edits, and deletes. A **60-minute TTL** evicts stale entries so long-running sessions never serve outdated dir listings.
- **Binary file guard** â€” `read_file` rejects files larger than 50 MB or detected as binary (PNG, JPEG, PDF, ZIP, ELF, PE/EXE) with a clear error message. Use `search_in_file` to locate specific content in large or binary files instead.
- **MCP Health** status bar (`$(plug) MCP Connected` / `$(plug) MCP Â· N agents` / `$(warning) MCP: setup needed`) â€” click for the combined health report (filesystem + CLI sections).
- **Agent KPI** status bar (`$(pulse) KPI: A Â· 42 calls`) â€” live efficiency grade from the last 24 h of file-access telemetry.
- **MCP Force Mode** â€” blocks Claude's native `Read`/`Write`/`Edit`/`Glob`/`Grep`/`Bash` **and** the CLI MCP tools (`run_command`, `list_available_clis`), routing all file I/O through the filesystem MCP server. Enable at startup via `claudeSkills.mcpForce.enableOnStartup`. Revert with **Disable MCP Force Mode**.
- **Apply auto-fixes to hints** â€” button in the Efficiency panel (and Command Palette â†’ **Claude Skills: Apply MCP Auto-Fixes**) that converts detected hot files, excessive scans, and large-file patterns into permanent cache rules in `mcp-agent-hints.md`. Rules survive session-hint refreshes.
- **Clear MCP Logs** â€” Command Palette â†’ **Claude Skills: Clear MCP Server Logs**.
- **Dir Cache Guard** â€” PreToolUse hook that blocks redundant `list_directory` calls within a session using an in-memory cache. Auto-installed with the server.
- **Log auto-pruning** â€” `mcp-usage.jsonl` is trimmed automatically when it exceeds 2 MB (keeps the last 30 days), using an atomic temp-file rename to avoid interrupting concurrent appends.
...
[skill-gap-detector] Skills missing for this workspace:
  â€¢ azure-infra-preflight â€” Azure provider (azurerm) found in Terraform files
  â€¢ terraform-plan-review â€” Terraform (.tf) files detected in workspace
...
The hook outputs nothing when all relevant skills are installed â€” zero noise in normal sessions. It self-heals its own path on extension updates (same mechanism as `terminal-watch`).
...
> **v1.0.85 — v1.1 UX Modernization: unified intelligence platform** — Status bar redesigned from 8 bars to 3 signal bars (API Score · Cost · Attribution alert). Executive Summary panel answers "how am I doing?" on every dashboard open. Dashboard reorganized into 9 collapsible `<details>` sections. New: Learning Timeline (`learningTimeline.ts`), Prediction Intelligence (per-skill precision/recall/F1), Governance compliance checklist, Adaptation Log (`adaptation-log.jsonl`), Feature Modes (`starter`/`professional`/`power`/`team`). Three toggle commands replace 6 enable/disable pairs: `toggleMcpForce`, `manageMcpServers`, `manageEfficiencyGuards`. 463/463 tests pass. 4.28 MB VSIX (614 files).

> **v1.0.84 — Attribution stale-port fix, proposal history boost, cost-control hooks auto-enabled, profile/manifest cleanup, Agent Performance Index, audit CSV export** — `ensurePostToolHookRegistered` now replaces stale-port commands (51710 → 4895) in-place. Proposal engine applies +25 (7-day) or +15 (30-day) history boost; catch-all globs no longer inflate scores. Cost-control hooks auto-install on first workspace open. Archived skills filtered from profile; manifest/package/readme/changelog/license added to SKILL_DENYLIST. New 0–100 Agent Performance Index panel. `Claude Skills: Export Skill Telemetry (CSV)` command added. 463/463 tests pass. 4.27 MB VSIX.

> **v1.0.83 — MCP `edit_file` CRLF fix, stop-word proposal filter, skill library cleanup** — `edit_file` now succeeds on Windows CRLF YAML files (normalizes `\r\n` before matching, restores original line endings). Proposal engine stop-word filter expanded from 7 to ~55 English words — eliminates confidence=100 false positives for unrelated skills. Skill manifest cleaned: 4 creative skills archived, `ci-pipeline-debug`+`ci-preflight` merged into `github-actions-ci`, `azure-infra-preflight` globs tightened. Two new skills: `vitest-extension-testing` and `github-actions-ci`. 453 tests pass (+5).

> **v1.0.80 â€” HACE score (Human-AI Collaboration Efficiency)** â€” New metric computed from Claude session transcripts (`~/.claude/projects/`) combined with CLI telemetry. Four components: Prompt Clarity (thinking-block rate), Task Velocity (turns/min), Accuracy Rate (correction-turn rate), CLI Efficiency (exit-code success rate). Composite weighted score 0â€“100 with letter grade, visible in the Efficiency Metrics dashboard panel and plain-text report. Session JSONL is parsed per-workspace; the panel is silent when no transcript data exists.

> **v1.0.79 â€” terminal-watch auto-install on new workspaces** â€” `terminal-watch` (the PostToolUse hook that writes CLI exit codes to `mcp-usage.jsonl`) was previously only installed when a user explicitly ran "Enable Cost Control Hooks". On a brand-new workspace with no prior setup, no hook fired and `mcp-usage.jsonl` was never created. Fixed by wiring `installTerminalWatchHook` into the automatic activation path alongside the skill-gap-detector, so telemetry starts on first workspace open without any manual step. A standalone `installTerminalWatchHook` export was also added so the hook can be installed independently of the full cost-control suite.

> **v1.0.78 â€” Skill gap detector + terminal-watch self-heal** â€” New `SessionStart` hook (`skill-gap-detector.js`) scans the workspace at startup, detects missing skills for the active tech stack (Terraform, Azure, shell scripts, security), and injects a plain-text warning into the agent session before any tool call. Zero output when all skills are present. Additionally, `terminal-watch` now self-heals its extension path on version upgrades â€” previously the hook silently broke whenever the extension updated because the versioned path in `settings.json` was not updated automatically.

> **v1.0.73 â€” Native tool operations now logged** â€” PostToolUse hooks now capture all IDE tool invocations, including native tools like `run_task` and `run_in_terminal`. These appear in `mcp-usage.jsonl` with the `tool` field prefixed as `native:<toolName>` to distinguish them from MCP server operations. This provides complete observability across all agent tooling, filling a previous gap where native tools were untracked.
...
- The **Agent KPI** status bar shows an efficiency grade (Aâ€“F) based on how the agent used file and IDE tools in the last 24 h (including native `run_task`, `run_in_terminal`, etc.).
- The **Cost Dashboard â†’ Efficiency metrics** panel breaks down token waste by tool, file, and session.
...
- KPI grades are meaningful and comprehensive (all tool calls are trackedâ€”no gaps).
...
- **Indexed stats** â€” `skill-stats.json` + `dashboard-snapshot.json` updated on refresh (reduces full `runs.jsonl` scans); in-memory cache on mtime/size
...
- **Disk**: ~500 KBâ€“2 MB per project under `<workspace>/.claude/learning/` (`runs.jsonl`, indexes, attribution store); `skill-stats.json` / `dashboard-snapshot.json` limit full-log rescans
